  ...
  const load = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setError("Could not retrieve authors.");
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-800 text-slate-100 px-4 pb-12 antialiased">
      <div className="pt-20 sm:pt-24">
        ...
        {isLoaded && !error && authors.length === 0 && (
          <div className="max-w-xl mx-auto bg-slate-800/70 text-slate-300 px-6 py-8 rounded-lg text-center shadow-xl">
            <h2 className="text-2xl font-semibold text-sky-400 mb-3">No Authors Found</h2>
            <p className="mb-6">There are no authors in the collection yet.</p>
            <Link
              to="/createauthor"
